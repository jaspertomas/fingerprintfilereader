...
//    }
    public void generate(String yearstring) {
        try {
            //Regular holidays
            allItems.add(new Holiday("New Year",Adjustments.REGULAR,dateFormat.parse(yearstring+"-01-01")));
            allItems.add(new Holiday("Araw ng Kagitingan",Adjustments.REGULAR,dateFormat.parse(yearstring+"-4-9")));
            allItems.add(new Holiday("Labor Day",Adjustments.REGULAR,dateFormat.parse(yearstring+"-5-1")));
            allItems.add(new Holiday("Independence Day",Adjustments.REGULAR,dateFormat.parse(yearstring+"-6-12")));
            allItems.add(new Holiday("Bonifacio Day",Adjustments.REGULAR,dateFormat.parse(yearstring+"-11-30")));
            allItems.add(new Holiday("Christmas",Adjustments.REGULAR,dateFormat.parse(yearstring+"-12-25")));
            allItems.add(new Holiday("Rizal Day",Adjustments.REGULAR,dateFormat.parse(yearstring+"-12-30")));

            //movable regular holidays for 2014
            if(yearstring.contentEquals("2014"))
            {
                allItems.add(new Holiday("Maundy Thursday",Adjustments.REGULAR,dateFormat.parse(yearstring+"-4-17")));
                allItems.add(new Holiday("Good Friday",Adjustments.REGULAR,dateFormat.parse(yearstring+"-4-18")));
                allItems.add(new Holiday("National Heroes' Day",Adjustments.REGULAR,dateFormat.parse(yearstring+"-8-25")));
            }

            //Special non-working holidays
            allItems.add(new Holiday("Ninoy Aquino Day",Adjustments.SPECIAL,dateFormat.parse(yearstring+"-8-21")));
            allItems.add(new Holiday("All Saints' Day",Adjustments.SPECIAL,dateFormat.parse(yearstring+"-11-1")));
//            allItems.add(new Holiday("All Souls' Day",Holidays.SPECIAL,dateFormat.parse(yearstring+"-11-2")));
            allItems.add(new Holiday("Christmas Eve",Adjustments.SPECIAL,dateFormat.parse(yearstring+"-12-24")));
            allItems.add(new Holiday("Day after Christmas",Adjustments.SPECIAL,dateFormat.parse(yearstring+"-12-26")));
            allItems.add(new Holiday("Last Day of the Year",Adjustments.SPECIAL,dateFormat.parse(yearstring+"-12-31")));

            //movable special holidays for 2014
            if(yearstring.contentEquals("2014"))
            {
                allItems.add(new Holiday("Chinese New Year",Adjustments.SPECIAL,dateFormat.parse(yearstring+"-1-31")));
                allItems.add(new Holiday("Black Saturday",Adjustments.SPECIAL,dateFormat.parse(yearstring+"-4-19")));
            }
